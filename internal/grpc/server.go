...
//
// attestSvc is optional: if non-nil, the server enriches signed report
// responses with the current SEV-SNP launch measurement so clients can
// prove the report was generated by a specific audited build.
func NewServer(
	logger *zap.Logger,
	connSvc connectionService,
	syncSvc *service.SyncService,
...
func (s *Server) GenerateSignedReport(ctx context.Context, req *pb.ReportRequest) (*pb.SignedReportResponse, error) {
	...
	// Attach the current SEV-SNP launch measurement so clients can bind
	// the report signature to the audited enclave build. This is the
	// bridge that transforms "valid ECDSA signature over some data" into
	// "report produced by the enclave at hash X on AMD SEV-SNP hardware".
	// The attestation service caches its result for 5 seconds so this
	// is effectively free on back-to-back report generation.
	if s.attestSvc != nil {
		attest, attestErr := s.attestSvc.GetAttestation(ctx)
		if attestErr != nil {
			s.logger.Warn("failed to fetch attestation for signed report (measurement will be empty)",
				zap.Error(attestErr),
			)
		} else if attest != nil && attest.Attestation != nil {
			resp.Measurement = attest.Attestation.Measurement
		}
	}
