...
║  │  - Manual syncs blocked after initial connection     │ ║
║  └───────────────────────────────────────────────────────┘ ║
║                                                             ║
║  ┌───────────────────────────────────────────────────────┐ ║
...
│  │  3. Block manual syncs (auto-only after init)    │   │
...
2. **Autonomous Systematic Snapshots**: The scheduler runs inside the hardware-attested enclave at 00:00 UTC daily, ensuring snapshots are taken systematically and cannot be cherry-picked at favorable market conditions. Manual syncs are blocked after the initial connection — all subsequent syncs are automatic only.

3. **Audit Trail**: All sync operations are logged with timestamps in the snapshot_data table, proving that snapshots were created systematically by the enclave scheduler, not manually triggered to hide losses.
...
| **Total** | **39** | **~6,200** | Minimized attack surface |

**Rationale for TCB size**: By isolating only the code that MUST handle credentials and create snapshots, we reduce the attack surface from ~12,000 LOC (full platform) to ~6,200 LOC (enclave only). This makes security audits tractable and reduces the probability of vulnerabilities. The autonomous scheduler is included in the TCB to prove snapshot integrity via hardware attestation.
...
- [ ] Verify manual syncs are blocked after initial connection (anti cherry-picking)
